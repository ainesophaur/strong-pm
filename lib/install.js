...
var dbUpgrade = require('./upgrade-db');
...
function install(argv, callback) {
  ...
  while ((option = parser.getopt()) !== undefined) {
    switch (option.option) {
      ...
      case 'I': // --skip-default-install
        ...
        break;
      case 'M':
        jobConfig.useMemoryDb = true;
        jobConfig.dbUpdate = false;
        break;
      default:
  ...
  if (jobConfig.useMemoryDb) {
    jobConfig.command.push('--in-memory-db');
  }
...
function checkAndUpgradeDb(options, callback) {
  var memoryDbLocation = options.env.STRONGLOOP_MESH_DB ||
    path.join(options.pmBaseDir, 'strong-mesh.json');
  fs.stat(memoryDbLocation, function(err) {
    if (err) return callback();
    dbUpgrade(options.pmBaseDir, memoryDbLocation, options.dryRun, callback);
  });
}
